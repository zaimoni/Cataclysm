...
std::optional<std::vector<point> > reality_bubble::toScreen(const std::vector<GPS_loc>& src) const
{
	std::vector<point> ret;
	for (decltype(auto) loc : src) {
		if (auto pos = toScreen(loc)) ret.push_back(*pos);
		else break;
	};
