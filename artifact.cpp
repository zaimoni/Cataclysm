...
// If we have charges, pick a recharge mechanism
  ...
  art->id = item::types.size();
  item::types.push_back(art);
  return art;
  ...
  if (it->charges < tool->max_charges) {
   ...
   switch (tool->charge_type) {
    case ARTC_TIME:
     ...
     break;
    case ARTC_SOLAR:
     if (messages.turn.second == 0 && messages.turn.minute % 10 == 0 && is_in_sunlight(p->GPSpos))
      it->charges++;
     break;
    case ARTC_PAIN:
  ...
  case AEP_EXTINGUISH:
   for (int x = p->pos.x - 1; x <= p->pos.x + 1; x++) {
    for (int y = p->pos.y - 1; y <= p->pos.y + 1; y++) {
     ...
     if (fd.type == fd_fire) {
      if (1 >= fd.density) m.remove_field(x, y);
      else fd.density--;
     }
    }
   }
