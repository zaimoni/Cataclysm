...
void map::init()
{
 SET_VECTOR(items[mi_field], itm_rock, itm_strawberries);

 SET_VECTOR(items[mi_forest],
	itm_rock, itm_stick, itm_mushroom, itm_mushroom_poison,
	itm_mushroom_magic, itm_blueberries);

 SET_VECTOR(items[mi_hive], itm_honeycomb);
 SET_VECTOR(items[mi_hive_center], itm_honeycomb, itm_royal_jelly);

 SET_VECTOR(items[mi_road],
	itm_muffler, itm_pipe, itm_motor, itm_wheel, itm_big_wheel, itm_seat, 
    itm_combustion_small, itm_combustion);

 SET_VECTOR(items[mi_livingroom],
 ...
 SET_VECTOR(items[mi_cannedfood],
 ...
 SET_VECTOR(items[mi_consumer_electronics],
 ...
 SET_VECTOR(items[mi_allsporting],
 ...
 SET_VECTOR(items[mi_alcohol], itm_whiskey, itm_vodka, itm_rum, itm_tequila);
 SET_VECTOR(items[mi_pool_table], itm_pool_cue, itm_pool_ball);

 SET_VECTOR(items[mi_trash],
 ...
 SET_VECTOR(items[mi_bags], itm_backpack, itm_purse, itm_mbag);

 SET_VECTOR(items[mi_allclothes],
 ...
 SET_VECTOR(items[mi_cop_weapons],
 ...
 SET_VECTOR(items[mi_cop_evidence],
 ...
 SET_VECTOR(items[mi_hospital_lab],
 ...
 SET_VECTOR(items[mi_mil_surplus], // NOT food or armor!
 ...
 SET_VECTOR(items[mi_hydro],
	itm_blueberries, itm_strawberries, itm_tomato, itm_broccoli,
	itm_zucchini, itm_potato_raw);

 SET_VECTOR(items[mi_electronics],
 ...
 SET_VECTOR(items[mi_bionics_common],
	itm_bionics_battery, itm_bionics_power, itm_bionics_tools);

 SET_VECTOR(items[mi_bots], itm_bot_manhack, itm_bot_turret);

 SET_VECTOR(items[mi_launchers],
 ...
 SET_VECTOR(items[mi_mil_rifles],
 ...
 SET_VECTOR(items[mi_survival_armor],
 ...
 SET_VECTOR(items[mi_survival_tools],
 ...
 SET_VECTOR(items[mi_sewage_plant],
 ...
 SET_VECTOR(items[mi_mine_storage], itm_rock, itm_coal);

 SET_VECTOR(items[mi_mine_equipment],
 ...
 SET_VECTOR(items[mi_spiral], itm_spiral_stone, itm_vortex_stone);

 SET_VECTOR(items[mi_radio],
 ...
 SET_VECTOR(items[mi_toxic_dump_equipment],
 ...
 SET_VECTOR(items[mi_ant_egg], itm_ant_egg);	//TODO: More items here?
 SET_VECTOR(items[mi_biollante], itm_biollante_bud);
 SET_VECTOR(items[mi_bugs], itm_chitin_piece);
 SET_VECTOR(items[mi_bees], itm_bee_sting, itm_chitin_piece);
 SET_VECTOR(items[mi_wasps], itm_wasp_sting, itm_chitin_piece);

 SET_VECTOR(items[mi_robots],
 ...
 SET_VECTOR(items[mi_helicopter],
...
// TODO: Replace kevlar with the ceramic plate armor
 ...
 SET_VECTOR(items[mi_stash_wood], itm_stick, itm_ax, itm_saw, itm_2x4);

 SET_VECTOR(items[mi_stash_drugs],
	itm_pills_sleep, itm_oxycodone, itm_xanax, itm_adderall, itm_weed,
	itm_coke, itm_meth, itm_heroin);

 SET_VECTOR(items[mi_drugdealer],
...
// This one kind of an inverted list; what an NPC will NOT carry
 SET_VECTOR(items[mi_trader_avoid],
